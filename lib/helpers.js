...
module.exports.dynamic = {
    ...
    route: function (req, res) {
        ...
        route.viewDir = req._locomotive.app.__controllers[route.controller].__viewDir;
        return route
    },
    messages: messages
};
...
function createCsrf(req) {
    if (req.session && req.session.csrf_token) {
        console.log('CSRF');
        return {'_csrf' : fields.string({ value: req.session.csrf_token, widget: widgets.hidden() })}
    }
    return {}
}
